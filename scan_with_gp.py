...
def gs2_growth_rate(beta, shat, base):
    ...
    run_gs2(run.run_directory, run.file_name)
...
y = [gs2_growth_rate(*x_, pyro) for x_ in x]
bounds = [(-1, 0), (0.01, 5)]
...
for _ in tqdm.tqdm(range(N**2 - 2)):
    ...
    new_y = gs2_growth_rate(new_x[0], new_x[1], pyro)
